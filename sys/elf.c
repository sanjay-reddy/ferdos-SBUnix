...
void elf_load(struct Task *task, char *file_name){
   ...
   if(addr!=0){
	  ...
	  task->regs.rip = elfHeader->e_entry;
          int ph_cnt = elfHeader->e_phnum;
       	  for(int i=0;i<ph_cnt;i++,ph1++){		
          	if(ph1->p_type==1){
                       vma_struct *vma = (vma_struct *)addVMA(elfHeader,ph1->p_vaddr,ph1->p_vaddr + ph1->p_memsz,ph1->p_flags,ph1->p_offset,NOTYPE,ph1->p_filesz);
	...
	addVMA(NULL,HEAP_START,HEAP_START + PAGE_SIZE,PERM_R | PERM_W,(uint64_t)NULL,HEAP,(uint64_t)NULL);

	//stack
	uint64_t *stack = (uint64_t *)get_phy_addr(USER_STACK_TOP);
	addVMA(NULL,(uint64_t)stack + PAGE_SIZE,(uint64_t)stack,PERM_R | PERM_W,(uint64_t)NULL,STACK,(uint64_t)NULL);
	task->regs.rsp = (uint64_t)((uint64_t)stack + 4096 - 8);
  }
}
